...
{
    private static readonly Dictionary<ProgressType, ProgressbarUpdate> _progressBars = new();
    ...
    private static void RuntimeArgumentHandle()
    {
        ...
        InitProgressBars();
        var config = Config.Load(cliOptions.ConfigFile);
        if (cliOptions.DryRun != null) config.DataSource.DryRun = cliOptions.DryRun.Value;

        if (string.IsNullOrEmpty(cliOptions.Locale)) config.DataGeneration.Locale = cliOptions.Locale;

        if (cliOptions.UpdateBatchSize != null) config.DataSource.UpdateBatchSize = cliOptions.UpdateBatchSize;
    ...
    private static void WriteLine(
        string message = null)
    {
        if (!cliOptions.NoOutput) Console.WriteLine(message);
    }
        ...
        Config config)
    {
        ...
        {
            var tableConfig = config.Tables[i];


            var rowCount = dataSource.GetCount(tableConfig);
            UpdateProgress(ProgressType.Masking, 0, rowCount, "Masking Progress");
            UpdateProgress(ProgressType.Updating, 0, rowCount, "Update Progress");
            ...
            //update all rows
            dataSource.UpdateRows(maskedRows, rowCount, tableConfig,
                totalUpdated => UpdateProgress(ProgressType.Updating, totalUpdated));
